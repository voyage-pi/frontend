...
const FriendsInvite = ({
    ...
    const fetchFriends = async () => {
      ...
      try {
        ...
        const friendsPromises = friendsData.map(async (friend) => {
          try {
            const userRes = await axiosUser.get(
              `/user/${friend.friend_id || friend.id}`
            );
        ...
        const friendsList = (await Promise.all(friendsPromises)).filter(
          Boolean
        );
      ...
      friends.map((friend) => {
        if (friend.id === id) {
          ...
          if (newSelected) {
            ...
            setAddedUsers((prev) =>
              prev.some((u) => u.id === friend.id) ? prev : [...prev, friend]
            );
          } else {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      const res = await axiosUser.get(
        `/friends/search?term=${encodeURIComponent(searchTerm)}`
      );
      let users = res.data || [];
  ...
  const handleAddUser = (user) => {
    ...
    if (typeof user === "string") {
      userObj =
        friends.find((f) => f.tag === user) ||
        searchResults.find((u) => u.tag === user);
      if (!userObj) {
        userObj = {
          id: user,
          name: user,
    ...
    if (userObj && !addedUsers.some((u) => u.id === userObj.id)) {
      setAddedUsers((prev) =>
        prev.some((u) => u.id === userObj.id) ? prev : [...prev, userObj]
      );
      setFriends((friends) =>
        friends.map((friend) =>
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollbarContainer.style.setProperty(
        "--scroll-thumb-height",
        `${thumbHeightPercentage}%`
      );
  ...
  useEffect(() => {
    if (setDisableButton) {
      setDisableButton(addedUsers.length === 0);
    }
  }, [addedUsers]);
  ...
  return (
    <div className="text-center p-6 -mb-10 flex flex-col min-h-[600px] relative">
          ...
          >
            {loading ? (
              <>
                ...
                >
                  <div className="skeleton w-25 h-25 px-3 py-1 rounded-full text-sm flex items-center"></div>
                  <div className="skeleton w-25 h-25 px-3 py-1 rounded-full text-sm flex items-center"></div>
                  <div className="skeleton w-25 h-25 px-3 py-1 rounded-full text-sm flex items-center"></div>
                </div>
              </>
                ...
                {friends.map((friend) => (
                  ...
                  >
                    <div className="relative mb-2">
                      ...
                      />
                      <button
                        className={`absolute -top-3 -right-2 w-6 h-6 rounded-full flex items-center justify-center ${
            ...
            <div className="flex flex-wrap gap-2">
              {addedUsers.length === 0 ? (
                ...
                addedUsers.map((user, index) => (
                  <span
                    key={user.id}
                    className="bg-rose-100 text-primary px-3 py-1 rounded-full text-sm flex items-center"
                  >
                    <img
                      src={user.image}
                      alt={user.name}
      ...
      <div className="flex justify-start w-full absolute bottom-0 left-0 px-6">
        <button
          onClick={onBack}
